...
struct Polar <: Gadfly.CoordinateElement
    xvars::Vector{Symbol}
    yvars::Vector{Symbol}
    xmin
    xmax
    ...
    Polar(xvars, yvars, xmin, xmax, ymin, ymax, xflip, yflip, fixed, aspect_ratio, raster) =
            new(xvars, yvars, xmin, xmax, ymin, ymax, xflip, yflip, fixed,
                isa(aspect_ratio, Real) ? Float64(aspect_ratio) : aspect_ratio, raster)
end
...
"""
    Coord.polar(; xmin=nothing, xmax=nothing, ymin=nothing, ymax=nothing,
                    xflip=false, yflip=false,
                    aspect_ratio=nothing, fixed=false,
                    raster=false)
